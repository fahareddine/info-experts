...
export function sendAppointmentStatusEmail(record, action) {
  ...
  const extra = action === 'confirm'
    ? '<div style="padding:12px 16px;background:#eff6ff;border-radius:8px;font-size:13px;color:#1d4ed8">Nous vous attendons au créneau indiqué. Si vous avez une contrainte, répondez à cet email ou contactez-nous au +269 331 27 22.</div>'
    : action === 'cancel'
...
export function sendPaymentReviewEmail(record, action) {
  ...
  const extra = action === 'validate'
    ...
    : '<div style="padding:12px 16px;background:#fef2f2;border-radius:8px;font-size:13px;color:#991b1b">Merci de vérifier la référence transmise ou de nous contacter au +269 331 27 22 pour assistance.</div>';
  ...
  const html = wrap(`
    <div style="padding:24px 28px">
      ...
      <div style="padding:12px 16px;background:#fff7ed;border-radius:8px;font-size:13px;color:#9a3412">
        Besoin d'aide ? Contactez Info Experts au <strong>+269 331 27 22</strong> pour finaliser rapidement votre dossier.
      </div>
    </div>`, true);
  ...
  const html = wrap(`
    <div style="padding:24px 28px">
      ...
      <div style="padding:12px 16px;background:#f8fafc;border-radius:8px;font-size:12px;color:#6b7280">
        Conservez cette facture pour vos archives. Pour toute question, contactez-nous au <strong>+269 331 27 22</strong> ou par email à <strong>contact@info-experts.fr</strong>.
      </div>
    </div>`, true);
