...
export function sendOrderSms(record) {
  ...
  const body = record.paymentType === 'cash'
    ? `Info Experts - Commande reçue (${record.reference}). ${fmtKmf(record.amountKmf)}. Paiement en espèces à la remise. +269 331 27 22`
    : `Info Experts - Commande reçue (${record.reference}). ${fmtKmf(record.amountKmf)}. Vérification en cours. +269 331 27 22`;
...
export function sendAppointmentSms(record) {
  ...
  const body = record.paymentType === 'free'
    ? `Info Experts - RDV confirmé (${record.reference}) ${slot}. ${truncate(record.location, 30)}. +269 331 27 22`
    : record.paymentType === 'cash'
      ? `Info Experts - RDV réservé (${record.reference}) ${slot}. Paiement espèces sur place. +269 331 27 22`
      : `Info Experts - RDV reçu (${record.reference}) ${slot}. Vérification paiement en cours. +269 331 27 22`;
...
export function sendPaymentSms(record) {
  ...
  const body = `Info Experts - Paiement reçu (${record.reference}). ${fmtKmf(record.amountKmf)}. Vérification sous 15 min. +269 331 27 22`;
...
export function sendAppointmentStatusSms(record, action) {
  ...
  const slot = fmtDate(record.appointmentDate, record.appointmentTime);
  const messages = {
    confirm:   `Info Experts - RDV ${record.reference} confirmé ${slot}. Lieu : ${truncate(record.location, 25)}. +269 331 27 22`,
    cancel:    `Info Experts - RDV ${record.reference} annulé. Contactez-nous : +269 331 27 22`,
    complete:  `Info Experts - RDV ${record.reference} terminé. Merci pour votre confiance ! +269 331 27 22`,
  };
...
export function sendOrderStatusSms(record, action) {
  ...
  const messages = {
    validate: `Info Experts - Commande ${record.reference} validée. Notre équipe vous contactera pour la remise. +269 331 27 22`,
    cancel:   `Info Experts - Commande ${record.reference} annulée. Contactez-nous : +269 331 27 22`,
    fulfill:  `Info Experts - Commande ${record.reference} remise. Merci pour votre confiance ! +269 331 27 22`,
  };
...
export function sendPaymentReviewSms(record, action) {
  ...
  const body = action === 'validate'
    ? `Info Experts - Paiement ${record.reference} validé. Merci ! +269 331 27 22`
    : `Info Experts - Paiement ${record.reference} non validé. Contactez-nous : +269 331 27 22`;
...
export function sendPaymentReminderSms(record) {
  ...
  const body = `Info Experts - Rappel : paiement ${record.reference} (${fmtKmf(record.amountKmf)}) toujours en attente. Contactez-nous : +269 331 27 22`;
