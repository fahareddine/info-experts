...
async function handleLogin(req, res) {
  ...
  await checkRateLimit(req, 'admin-login', 10);
...
async function loadDetailRecord(entity, reference) {
  ...
  if (entity === 'customers') {
    ...
    const orFilters = [];
    if (email) orFilters.push(`customer_email.eq.${escapePostgrestString(email)}`);
    if (phone) orFilters.push(`customer_phone.eq.${escapePostgrestString(phone)}`);
    if (!orFilters.length) {
      orFilters.push(`customer_full_name.eq.${escapePostgrestString(record.display_name)}`);
    }
    ...
    const [customerAppointments, customerOrders, customerPayments, timeline] = await Promise.all([
      ...
      selectRows('payments', { filters: { or: `(${[
        email ? `customer_email.eq.${escapePostgrestString(email)}` : null,
        phone ? `customer_phone.eq.${escapePostgrestString(phone)}` : null,
        `customer_full_name.eq.${escapePostgrestString(record.display_name)}`,
